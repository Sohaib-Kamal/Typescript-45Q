...
function test_if_name_is_lowercase(ur_name) {
...
if (tom == "cat" && casper == "cat") {
    console.log('Tom & Casper are cats');
}
else {
    console.log('one of them is not a cat');
}
console.log("test 2 for 'and' & 'or' oprator");
console.log('is tom = "cat" || jerry ="cat". i predict true');
if (tom == "cat" || jerry == "cat") {
    console.log('Either one of tom or jerry is a cat');
}
else {
    console.log('Both Tom and jerry are not cats');
}
// • Test whether an item is in a array
// • Test whether an item is not in a array
let stationary = ["pencil", "eraser", 'sharpner', 'scale', 'highlighter'];
function search_array(searchterm) {
    if (stationary.includes(searchterm)) {
        console.log(searchterm, 'is in the array');
    }
    else {
        console.log(searchterm, 'is not in the array.');
    }
}
